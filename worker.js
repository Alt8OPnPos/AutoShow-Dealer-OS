...
<header>
  <a href="/" class="logo">AUTOSHOW<span>.</span></a>
...
async function servePhoto(path, env) {
  ...
  // Photos are uploaded via the Cloudflare dashboard (R2 -> autoshow-vehicle-photos
  // -> Upload) - no code change needed to add new photos, just upload and
  // reference the filename as the key in D1's photo_urls field.
...
async function renderVehiclePage(path, env, url) {
  ...
  const schema = {
    ...
    "offers": {
      ...
      "price": price,
      "priceCurrency": "ZAR",
      "availability": item.status === "available" ? "https://schema.org/InStock" : "https://schema.org/
